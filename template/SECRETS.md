...
Which model: set repository **variables** (not secrets) — `LLM_PROVIDER` = `anthropic` (default) · `openai` · `gemini` · `openai-compatible`; `LLM_MODEL` = a model id (required unless you use Claude, which defaults to `claude-sonnet-5`); `LLM_BASE_URL` for an OpenAI-compatible server. The script calls each API directly over HTTPS (`mail/scripts/llm.mjs`, no SDK) and sends keys in headers only.
